...
# Check if WEBHOOK_PATH exists (required for production)
if ! grep -q "WEBHOOK_PATH" .env; then
    echo -e "${YELLOW}Adding WEBHOOK_PATH to .env...${NC}"
    # Add after WEBHOOK_URL line
    if grep -q "WEBHOOK_URL" .env; then
        sed -i '/WEBHOOK_URL/a WEBHOOK_PATH=/webhook' .env
        echo -e "${GREEN}✅ WEBHOOK_PATH added${NC}"
    else
        echo -e "${RED}❌ WEBHOOK_URL not found in .env!${NC}"
        echo "Please add WEBHOOK_URL and WEBHOOK_PATH manually"
        exit 1
    fi
fi
...
if grep -q "ngrok" .env; then
    echo -e "${YELLOW}⚠️  WARNING: ngrok detected in .env!${NC}"
    echo -e "${YELLOW}⚠️  For production, use your domain: https://bot.macone.net${NC}"
    echo -e "${YELLOW}⚠️  See PRODUCTION_WEBHOOK_DEPLOY.md for details${NC}"
    echo ""
fi
